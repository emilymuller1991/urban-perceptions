...
def get_panoids(x, y, all_gps, all_pano, api_key):
    ...
    # obtain list of panoids over years
    # [{'panoid': 'id', 'lat': lat, 'lon': lon, 'year': 2008, 'month': 7}]
            ...
            if "year" in pano.keys():
                try:
                    ...
                    if owner == "Google" and status == "OK":
                        ...
                        all_pano[pano["panoid"]] = {
                            key: pano[key] for key in ["lat", "lon", "year", "month"]
                        }
                    else:
                        pass
                except Exception as e:
                    print("Pano is not added because of the following error %s" % e)
                    continue
            else:
                ...
                try:
                    ...
                    if owner == "Google" and status == "OK":
                        ...
                        all_gps[str(point)].append(pano["panoid"])
                        all_pano[pano["panoid"]] = {
                            key: pano[key] for key in ["lat", "lon"]
                        }
